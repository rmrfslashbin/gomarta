...
func (r *Route) Add(record []string) (int, error) {
	if len(record) != 9 {
		return -1, fmt.Errorf("invalid route record length: %d", len(record))
	}
	...
	if err != nil {
		return -1, fmt.Errorf("route id: %v", err)
	}

	if r.RouteType, err = strconv.Atoi(record[5]); err != nil {
		return -1, fmt.Errorf("route type: %v", err)
	}
	if color, err := hex.DecodeString(record[7]); err != nil {
		return -1, fmt.Errorf("route color: %v", err)
	} else {
		r.Color = color
	}
	if textColor, err := hex.DecodeString(record[8]); err != nil {
		return -1, fmt.Errorf("route text color: %v", err)
	} else {
